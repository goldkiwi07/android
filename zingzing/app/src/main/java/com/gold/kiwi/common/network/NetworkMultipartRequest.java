...
public class NetworkMultipartRequest
{
	...
	public NetworkMultipartRequest(Context context)
	{
		this.context = context;
		dataUtil = new DataUtil(context);
	}

	public NetworkMultipartRequest(Context context, NetworkHandler handler)
	{
		this(context);
	...
	public void setData(JSONObject data)
	{
		if(data != null)
		{
			try
			{
				...
				this.jsonData.put("requestURL", url);
				this.jsonData.put("category", "request");
				this.jsonData.put("packageName", context.getPackageName());
				this.jsonData.put("deviceId", dataUtil.getStringData("device_id", "no_device_id"));
				this.jsonData.put("userCode", dataUtil.getStringData("user_code", "no_user_code"));
			...
			throw new NullPointerException("SendData is Null!!");
	}

	public void setImage(String formName, Bitmap image)
	{
			...
			public void run()
			{
				...
				try
				{
					...
					if(conn != null)
					{
						...
						byte[] byteImage = bitmapToByteArray(image);
						Log.d(TAG, "요청 : " + formName + ", "+ byteImage.length);

						dos.writeBytes(twoHyphens + boundary + lineEnd);
						dos.writeBytes("Content-Disposition: form-data; name=\""+ formName +"\";filename=\""+ formName +".jpg\"" +lineEnd);
						...
						{
							temp = (String)jsonData.get(names.getString(i));

							dos.writeBytes(twoHyphens + boundary + lineEnd);
							dos.writeBytes("Content-Disposition: form-data; name=\"" + names.getString(i) + "\"" + lineEnd);
							dos.writeBytes(lineEnd);
							dos.writeBytes(temp);
							dos.writeBytes(lineEnd);
						}
					...
					if(sBuffer.toString().trim().equals("") || sBuffer.toString().equals("null"))
					{
